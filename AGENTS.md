...
11. Load the nine builtin schemas from `data/tools/*.md`, conditionally enable web and ComfyUI tools, construct the private background memory extractor, and construct durable formation and session-compaction workers; `mcp.Provider` creates discovery tools per request rather than registering them during bootstrap
...
- Five builtin model tools are always enabled; web and ComfyUI tools are optional, `global_memory_search` is the sole model-visible global-memory operation, and additional tools require MCP discovery or eligible recent-tool pre-exposure
