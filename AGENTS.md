...
- Discord progressively edits visible assistant content into a cursor-suffixed preview, finalizes prose at tool boundaries, and keeps one compact accumulated tool-status message; thinking, tool arguments, and tool results remain hidden. The completed response is authoritative and is reconciled into the preview before delivery succeeds
...
- Progressively edits model output into Discord messages, stopping the typing indicator after the first visible preview and splitting the authoritative final response at Discord's 2,000-character limit
