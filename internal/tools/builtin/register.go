...
func Register(reg *registry.Registry, cfg *config.Config, userMemStore *usermemory.Store, globalMemStore *globalmemory.Store, log *config.Logger) error {
	...
	if err := reg.RegisterHandler("web.search", toolPolicy(4, 2, normalizeSearchArgs), registry.Handler(websearch.NewHandler(searchClient, log))); err != nil {
	...
	if err := reg.RegisterHandler("time.current", toolPolicy(4, 1, normalizeTimeArgs), registry.Handler(currenttime.NewHandler(time.Now))); err != nil {
	...
	if err := reg.RegisterHandler(toolnames.UserMemorySearch, toolPolicy(4, 2, normalizeMemorySearchArgs(8)), registry.Handler(usermemory.NewSearchHandler(userMemStore, log))); err != nil {
	...
	if err := reg.RegisterHandler(toolnames.UserMemoryList, toolPolicy(2, 1, normalizeMemorySearchArgs(25)), registry.Handler(usermemory.NewListHandler(userMemStore, log))); err != nil {
	...
	if err := reg.RegisterHandler(toolnames.SessionTranscriptSearch, toolPolicy(4, 2, normalizeMemorySearchArgs(5)), registry.Handler(usermemory.NewTranscriptSearchHandler(userMemStore, log))); err != nil {
		return fmt.Errorf("failed to initialize %s tool: %w", toolnames.SessionTranscriptSearch, err)
	}

	if err := reg.RegisterHandler(toolnames.GlobalMemorySearch, toolPolicy(4, 2, normalizeMemorySearchArgs(globalmemory.DefaultSearchLimit)), registry.Handler(globalmemory.NewSearchHandler(globalMemStore, log))); err != nil {
...
func toolPolicy(maxExecutions, maxUnproductive int, normalize governance.ArgumentNormalizer) governance.ToolPolicy {
