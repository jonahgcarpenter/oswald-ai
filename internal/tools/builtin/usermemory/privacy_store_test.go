...
func TestUserErasureInvalidationSurvivesAccountCascade(t *testing.T) {
	...
	if len(invalidation.ExternalIdentities) != 1 || len(invalidation.SessionIDs) != 1 {
	...
	if err != nil || event == nil || !event.CloseConnections || len(event.ExternalIdentities) != 1 || len(event.SessionIDs) != 1 {
