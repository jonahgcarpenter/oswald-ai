...
func scrubMemoryTx(ctx context.Context, tx *sql.Tx, userID string, memoryID int64, requestID string, now time.Time) error {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE memory_evidence SET content = '', source_request_id = '', source_session_id = '', source_turn_id = NULL WHERE canonical_user_id = ? AND memory_id = ?`, userID, memoryID); err != nil {
		return err
	}
...
func scrubCandidateTx(ctx context.Context, tx *sql.Tx, userID string, candidateID int64, now time.Time) error {
	nowText := formatTime(now)
	if _, err := tx.ExecContext(ctx, `UPDATE memory_evidence SET content = '', source_request_id = '', source_session_id = '', source_turn_id = NULL WHERE canonical_user_id = ? AND candidate_id = ?`, userID, candidateID); err != nil {
		return err
	}
	...
	_, err := tx.ExecContext(ctx, `UPDATE memory_candidates SET statement = '', statement_key = 'deleted:' || id, evidence_summary = '', state = 'rejected', source_request_id = '', source_session_id = '', source_turn_id = NULL, extraction_model = '', explicit_tool_source = '', confirmation_session_id = '', confirmation_request_id = '', published_memory_id = NULL, supersedes_memory_id = NULL, decision_reason = 'privacy_delete', decided_at = ?, decided_by = 'privacy', updated_at = ? WHERE canonical_user_id = ? AND id = ?`, nowText, nowText, userID, candidateID)
	return err
}
...
func deleteAllMemoriesTx(ctx context.Context, tx *sql.Tx, userID, requestID string, now time.Time) error {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE memory_evidence SET content = '', source_request_id = '', source_session_id = '', source_turn_id = NULL WHERE canonical_user_id = ?`, userID); err != nil {
		return err
	}
