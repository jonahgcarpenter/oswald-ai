...
func TestRankDurableMemoriesHybridMergeAndWeights(t *testing.T) {
	...
	result := results[0]
	want := 0.45*0.6 + 0.25*0.8 + 0.10*0.8 + 0.10*0.8 + 0.10 + 0.025
	...
	if result.Topic != "oswald" || result.AuthorityAdjustment != 0.025 {
...
func TestRenderDurableMemoryRecallQuotesPromptInjection(t *testing.T) {
	...
	if decoded["confidence"] != 0.8 || decoded["provenance"] == nil {
