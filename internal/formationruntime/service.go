...
// MarkConfirmationPresented binds a pending candidate after successful delivery.
func (s *Service) MarkConfirmationPresented(ctx context.Context, userID, sessionID string, generation int, requestID string, candidateID int64) error {
	return s.store.MarkConfirmationPresented(ctx, userID, sessionID, generation, requestID, candidateID)
}
