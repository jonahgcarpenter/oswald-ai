...
func (s *Service) sweepAt(ctx context.Context, now time.Time) {
	...
	if s.log != nil {
		s.log.Info("maintenance.sweep.complete", "periodic maintenance sweep completed",
			...
			config.F("redacted_count", counts.AuditRowsRedacted+counts.FormationJobsRedacted+counts.CompactionJobsRedacted+counts.CandidatesRedacted+counts.SupersededMemoriesRedacted+counts.EventsRedacted),
			config.F("supersedes_link_cleared_count", counts.SupersedesLinksCleared),
